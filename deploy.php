...
// `queue` runs the workers the notification sends are dispatched to.
set('compose_services', 'nginx php postgres scheduler queue');
 ...
 *
 * The `touch` is not cosmetic. `php`, `scheduler` and `queue` share the same bind mounts,
 * including the single-file mount `shared/back/.env` → `/var/www/html/.env`,
 * whose parent `/var/www/html` is itself a bind mount of `<release>/back`. A
 * file mountpoint has to exist before it can be bound over, and `back/.env` is
 * gitignored, so it is missing from a fresh release and runc creates it — inside
 * that shared host directory. runc 1.3.3 replaced the old EEXIST-tolerant
 * `mknodat` with lookup-then-`O_CREAT|O_EXCL`, so when compose starts these
 * services concurrently and more than one takes the create branch, the losers die with
 *
 *   error mounting ".../shared/back/.env" to rootfs at "/var/www/html/.env":
 *   create mountpoint for /var/www/html/.env mount: make mountpoint
 *   "/var/www/html/.env": file exists
 *
 * — an intermittent failure, since it needs two inits inside the same window.
 * Creating the mountpoint here means no container ever takes that branch.
 */
set('compose_up', 'touch back/.env && docker compose up -d --build {{compose_services}}');
